...
export class PessoaService {
  ...
  protected convertDateFromClient(pessoa: IPessoa): IPessoa {
    return Object.assign({}, pessoa, {
      dataDeNascimento: pessoa.dataDeNascimento?.isValid() ? pessoa.dataDeNascimento.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dataDeNascimento = res.body.dataDeNascimento ? dayjs(res.body.dataDeNascimento) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((pessoa: IPessoa) => {
        pessoa.dataDeNascimento = pessoa.dataDeNascimento ? dayjs(pessoa.dataDeNascimento) : undefined;
      });
    }
    return res;
  }
